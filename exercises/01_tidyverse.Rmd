...
Creating a new variable in the tidyverse uses the `mutate()` function. The most basic use, would be something like this:

```{r}
dat_spect %>%
  select(id, sample, `360nm`, `370nm`) %>% # select just the first few columns, for now.
  mutate("Species"="Pelobates cultripes") %>% # add a new column with a character vector
  mutate("additive_values"=`360nm`+`370nm`) # add a new column using some sort of operation
```

Note that new columns are added at the end of the dataframe.
  

By looking at the `sample` column, we can see that there is some relevant data hidden in there. This variable seems to be composed of a sample number followed by D = Dorsal, V = Ventral. Let's see if we can extract that information and structure it into separate columns.
  
We can do this most easily with the canned `separate()` function, which is in a way a wrapper around mutate. 
...
As you can see, it has split the `sample` column into two new columns, has removed the original column and has positioned the new columns where the old one was.  

Up until now, we have manipulated data, but we have not actually greated any data. We can make those operations more permanent, by storing them as a new object. I generally don't recommend to overwrite the original object (i.e. saving with the same name) because this can lead to issues when going back up to earlier parts of code, so let's creat e new one here, and while we are at it, let's remove the `id` column that we don't want anymore.
...
As a note, if we want more control over how to separate or create new variables from existing ones when working with strings (characters), the tidyverse leans heavily on the `stringr` package. Most functions are self explanatory and start with "str_". These are very powerful functions because they work with `regular expressions` (patterns). Definitely worth getting familarized with, and we might see those pop up in the next tutorial. 
...
-   Learn by doing! As with any programming language, the best way to learn is to just get your hands dirty. Your regular google search should be "how do i _____ using the tidyverse?".
