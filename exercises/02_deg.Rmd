...
Before we go any further however, we need to do a better job at normalizing our count data *across* samples, so that:

1.  The variances for highly expressed genes are appriximatly similar to the variances for low expressed genes.
2.  Counts are normalized with respect to library size (i.e. correcting differences in sequencing depths).
